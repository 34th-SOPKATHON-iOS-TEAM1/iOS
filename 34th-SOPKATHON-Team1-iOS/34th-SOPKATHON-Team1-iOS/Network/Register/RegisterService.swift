...
extension RegisterService {
    ...
    func postToDo(todoContent: PostTodoRequestBody, completion: @escaping (NetworkResult<Any>) -> Void) {
        mainProvider.request(.postToDo(request: todoContent)) { result in
            switch result {
    ...
    func postNotToDo(todoContent: PostNotTodoRequestBody, completion: @escaping (NetworkResult<Any>) -> Void) {
        mainProvider.request(.postNotToDo(request: todoContent)) { result in
            switch result {
            case .success(let response):
                let statusCode = response.statusCode
                let data = response.data
